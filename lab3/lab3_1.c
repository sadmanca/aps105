...
Table 1 below shows the normal boiling points of several substances. Write a C
program that prompts the user for the observed boiling point of a substance in
degree Celsius (◦C) (as an integer), and identifies the substance if the
observed boiling point is within t ◦C of the expected boiling point, where t is
a threshold defined by the user. If the data input is not within t ◦C of any of
the boiling points in the table, the program should output the message: 

    Substance unknown.

Table 1: Substances and their normal boiling points. | Substance | Normal
boiling point |
|-----------|----------------------|
| Water     | 100                  | | Mercury   | 357                  | |
Copper    | 1187                 | | Silver    | 2193                 | | Gold
| 2660                 |
...
Note: you may assume that the entered values by the user are valid integers
within the range allowed by the int type. The test cases will only use threshold
values sufficiently small to yield unique answers.
*/

//
// APS105-W22 Lab 3 Part 1 - Substance Test
//
// This program identifies a substance if its boiling point (given by the user)
// is within a given threshold of the expected boiling point of various
// substances
//
